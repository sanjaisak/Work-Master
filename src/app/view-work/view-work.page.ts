...
export class ViewWorkPage implements OnInit {
  ...
  async getData() {
    await this.http
      .get<any>('https://work-master20.herokuapp.com/getTask')
      .subscribe((data) => {
        this.taskInfo = data;
        ...
        this.taskData = [];
        const dataBasedOnDat = this.taskInfo.filter(
          (eachtask) => eachtask.date === this.taskInfo[0].date
        );
        this.taskData.push({
          date: this.taskInfo[0].date,
          value: dataBasedOnDat,
        });
        for (let i = 1; i < Object.keys(this.taskInfo).length; i++) {
          if (this.taskInfo[i].date !== this.taskInfo[i - 1].date) {
            const dataBasedOnDate = this.taskInfo.filter(
              (eachtask) => eachtask.date === this.taskInfo[i].date
            );
            this.taskData.push({
              date: this.taskInfo[i].date,
              value: dataBasedOnDate,
            });
          }
        }
